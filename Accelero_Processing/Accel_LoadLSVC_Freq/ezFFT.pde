...
class ezFFT {
  ...
  ezFFT(int _fftSize, float sampleRate1) {
    fftSize = _fftSize;
    fft = new FFT( fftSize, sampleRate1 );
    fft.window(FFT.NONE);
  ...
  void drawSpectrogram(float scale, float upperBound, boolean bShowFFT) {
    //int h = ceil((height-150)/(HIGH_THLD1-LOW_THLD1));
    ...
    for (int i = 1; i < colmax-leftedge; i++) {
      for (int j = 0; j < rowmax; j++) {
        //stroke(255-map(FFTHist1[j][i+leftedge], 0, 10, 0, 255));
