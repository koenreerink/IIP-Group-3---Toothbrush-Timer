...
void drawFFTInfo(int x, int y, int fs) {
  ...
  textSize(fs);
  float fps = frameRate;
  float fNormL = map(LOW_THLD1, 0, numBins1, 0, sampleRate1/2);
  float fNormH = map(HIGH_THLD1, 0, numBins1, 0, sampleRate1/2);
  float overlapping = (fps>0 ? 1-(sampleRate1)/(numBins1*fps):0);
  text("[N]umber of Bands: "+numBands1+" [#"+LOW_THLD1+"("+nf(fNormL,0,1)+"Hz)"+" - #"+HIGH_THLD1+"("+nf(fNormH,0,1)+"Hz)]", x, y+0*fs);
  text("[S]ample Rate: "+nf(sampleRate1, 0, 0)+" Hz", x, y+1*fs);
  text("[B]in Number(total): "+numBins1, x, y+2*fs);
  text("[F]rame Rate: "+nf(fps, 0, 2)+" fps", x, y+3*fs);
  text("Overlapping [1-S/BF]]: "+nf((float)overlapping*100, 0, 2)+" %", x, y+4*fs);
  popStyle();
}

//Draw a bar graph to visualize the modeArray1
