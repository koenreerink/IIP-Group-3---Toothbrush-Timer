...
String dataSetName = "ToothbrushTrain"; 
String[] attrNames;
...
void setup()
{
  ...
  loadTrainARFF(dataset="AllSounds.arff"); //load a ARFF dataset
  loadModel(model="AllSoundsLinearSVC.model"); //load a pretrained model.
 // loadModel(model=" C4Talk&ToothbrushLinearSVC.model"); //load a pretrained model.
}
...
void draw()
{
  ...
  //int arduino = myPort.read();

  //while (myPort.available() > 0 && arduino == 1) {
  // println("timer");
  // break;
 //  }
                      ...
                      energyMax = 0; //reset the me  asurement of energySum
                      ...
                      String Y = lastPredY;
                      //myPort.write(Y);         //send prediction to serialport
                      //println(Y);
